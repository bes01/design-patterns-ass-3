...
def test_get_type_method_1() -> None:
    assert (
        CMDParser.get_cmd_type("subscribe <Alice> to <Pewdiepie>")
        == CommandType.SUBSCRIBE
    )
    assert (
        CMDParser.get_cmd_type("subscribe <Jake> to <MrBeast>") == CommandType.SUBSCRIBE
    )


def test_get_type_method_2() -> None:
    assert (
        CMDParser.get_cmd_type("publish video on <MrBeast>")
        == CommandType.PUBLISH_VIDEO
    )
    assert (
        CMDParser.get_cmd_type("publish video on <Pewdiepie>")
        == CommandType.PUBLISH_VIDEO
    )
...
def test_parse_publish_video_cmd() -> None:
    ...
    assert (
        CMDParser.parse_publish_video_cmd("publish video on <Pewdiepie>") == "Pewdiepie"
    )
...
def test_parse_subscribe_cmd() -> None:
    ...
    subscriber, channel = CMDParser.parse_subscribe_cmd(
        "subscribe <Alice> to <Pewdiepie>"
    )
